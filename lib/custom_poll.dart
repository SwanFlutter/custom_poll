...
import 'package:custom_poll/src/polls.dart';
import 'package:custom_poll/src/radio_bottom_polls.dart';
import 'package:custom_poll/src/tools/utils/poll_data_utils.dart';
...
class CustomPoll extends StatefulWidget {
  ...
  static Widget polls({
    ...
    return Polls(title: title, options: options, votes: votes, totalVotes: totalVotes, onOptionSelected: onOptionSelected);
  }
...
class _CustomPollState extends State<CustomPoll> {
  ...
  void _selectOption(int index) {
    ...
    setState(() {
      ...
      if (selectedOption == index) {
        // Deselect or reduce vote
        widget.votes[index] = (widget.votes[index] ?? 0) - 1;
        selectedOption = null;
      } else {
        // If a previous option was selected, reduce its votes
        if (selectedOption != null) {
          widget.votes[selectedOption!] = (widget.votes[selectedOption!] ?? 0) - 1;
        }
